...
- **Pipe Pay** 1.8.3 - the plugin we sell, installed here as the same build customers receive. The Pipe Pay gateway itself is intentionally not enabled at WC checkout on pipepay.app (we sell licenses via Kestrel digital downloads, not P2P payments), but the plugin runs so we exercise the license activation, daily revalidation cron, and download-watermark flows on every release. As of v1.8.3 / Phase 5, the dogfood activates against itself with a real complimentary Unlimited license (order #220) — no more `PIPEPAY_DISABLE_LICENSING` bypass. To upgrade the dogfood install: easiest path is `sudo -u www-data wp --path=/var/www/pipepay plugin install /tmp/pipe-pay-vX.Y.Z.zip --force`. wp-cli does the deactivate/swap/reactivate dance for you and preserves option-stored settings.

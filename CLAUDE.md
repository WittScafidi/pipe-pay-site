...
- WooCommerce: 10.7.0
- Pipe Pay plugin: **1.8.1** on the dogfood gateway and wired into all four WC products (`_product_version=1.8.1`, `_downloadable_files` → `pipe-pay-v1.8.1.zip`). v1.8.1 adds Phase C anti-piracy (daily wp-cron revalidation against `/wp-json/pipepay-license/v1/revalidate` with three-state model `active|expired|revoked`, drain semantics on revoke, 30-day fail-open TTL, replay-resistant Ed25519 signature with `revalidate-v1|` canonical). v1.8.0 added Phase A + D (first-install gate, deterministic site fingerprinting). Public-facing site (release bar, changelog, footer ledger) still shows **v1.7.4** by design - `PIPEPAY_SITE_VERSION` reflects the most recent release with a published changelog narrative, not the actual dogfood version (per the docblock comment in `pipepay-child/functions.php`). The gap between public and dogfood is intentional: bug-fix and security patches between numbered "narrative" releases roll out to customers via auto-update without their own changelog entry. Plugin tags pushed up through `v1.8.1`. **Requires PHP 8.0** (was 7.4 pre-1.6.4). The dogfood install has `PIPEPAY_DISABLE_LICENSING` set in `wp-config.php` (it hosts the license server itself; can't license against itself). Source zips for every release live at `~/Desktop/Pipe Pay/pipe-pay-vX.Y.Z.zip`; v1.5.1 → v1.8.1 are also staged on the server at `/var/www/pipepay/wp-content/uploads/woocommerce_uploads/` for rollback. License-server URL is hardcoded to `https://pipepay.app/` in every build.

...
- WooCommerce: 10.7.0
- Pipe Pay plugin: **1.7.0** (updated 2026-05-08) — same version on the dogfood gateway AND in the customer-facing zip wired into the WC products. **Requires PHP 8.0** (was 7.4 pre-1.6.4). The dogfood install has `PIPEPAY_DISABLE_LICENSING` set in `wp-config.php` (it hosts the license server itself; can't license against itself). Source zip: `~/Desktop/Pipe Pay/pipe-pay-v1.7.0.zip`. License-server URL hardcoded to `https://pipepay.app/`. Staged on the server at `/var/www/pipepay/wp-content/uploads/woocommerce_uploads/pipe-pay-v1.7.0.zip` and wired into all four WC products via `_downloadable_files` + `_product_version=1.7.0`. Older zips (1.5.1 → 1.6.5) are still on disk in that uploads dir as rollback options but are not referenced by any product.
...
- Files:
  ...
  - `partials/logo-svg.php` — Pipe Pay logo SVG, single source of truth. Included from header.php, footer.php, and both spots in front-page.php (header + final-CTA inverse variant via `$pp_logo_variant = 'inverse'`).
...
## Local zips
- `/Users/wittscafidi/Desktop/Pipe Pay/pipe-pay-v1.7.0.zip` — **current customer-facing release AND what's installed on the dogfood gateway.** License-server response signing (Ed25519), structured `wc_get_logger()` logging, inline admin JS extracted to `assets/js/pipepay-admin.js`, i18n scaffolding. Wired into all 4 WC products. Also staged on the server at `/var/www/pipepay/wp-content/uploads/woocommerce_uploads/`.

...
- WooCommerce: 10.7.0
- Pipe Pay plugin: **1.8.2** on the dogfood gateway and wired into all four WC products (`_product_version=1.8.2`, `_downloadable_files` → `pipe-pay-v1.8.2.zip`). v1.8.2 adds Phase B anti-piracy (per-customer download watermarking via signed marker file at `pipe-pay/.pipepay-build` inside the zip; forensic decoder admin page on pipepay.app to identify leakers from a recovered zip). v1.8.1 added Phase C (daily wp-cron revalidation, three-state license model with drain semantics on revoke). v1.8.0 added Phase A + D (first-install gate, deterministic site fingerprinting). All four anti-piracy phases (A/B/C/D) are now shipped. Public-facing site (release bar, changelog, footer ledger) still shows **v1.7.4** by design - `PIPEPAY_SITE_VERSION` reflects the most recent release with a published changelog narrative, not the actual dogfood version (per the docblock comment in `pipepay-child/functions.php`). The gap between public and dogfood is intentional: bug-fix and security patches between numbered "narrative" releases roll out to customers via auto-update without their own changelog entry. Plugin tags pushed up through `v1.8.2`. **Requires PHP 8.0** (was 7.4 pre-1.6.4). The dogfood install has `PIPEPAY_DISABLE_LICENSING` set in `wp-config.php` (it hosts the license server itself; can't license against itself). Source zips for every release live at `~/Desktop/Pipe Pay/pipe-pay-vX.Y.Z.zip`; v1.5.1 → v1.8.2 are also staged on the server at `/var/www/pipepay/wp-content/uploads/woocommerce_uploads/` for rollback. License-server URL is hardcoded to `https://pipepay.app/` in every build.

...
- WooCommerce: 10.7.0
- Pipe Pay plugin: **1.6.4** (updated 2026-05-07) — same version on the dogfood gateway AND in the customer-facing zip wired into the WC products. **Requires PHP 8.0** (was 7.4 pre-1.6.4). The dogfood install has `PIPEPAY_DISABLE_LICENSING` set in `wp-config.php` (it hosts the license server itself; can't license against itself). Source zip: `~/Desktop/Pipe Pay/pipe-pay-v1.6.4.zip`. License-server URL hardcoded to `https://pipepay.app/`. Staged on the server at `/var/www/pipepay/wp-content/uploads/woocommerce_uploads/pipe-pay-v1.6.4.zip` and wired into all four WC products via `_downloadable_files` + `_product_version=1.6.4`. Older zips (1.5.1 → 1.6.3) are still on disk in that uploads dir as rollback options but are not referenced by any product.
...
- [ ] **End-to-end license activation test for v1.6.4.** All four WC products (34/35/36/38) point at `pipe-pay-v1.6.4.zip` with `_product_version=1.6.4` (done 2026-05-07). Existing 1.5.x/1.6.x customers will see "Update available" within ~12h of next WP cron. Smoke test before publicly recommending the upgrade:
  1. Buy a Pipe Pay tier (or manually create an order + mark it Processing) so API Manager issues a real license key.
  2. On a separate test WC install (PHP 8.0+ — 1.6.4 requires it), install `pipe-pay-v1.6.4.zip` (do NOT set `PIPEPAY_DISABLE_LICENSING` on the test site — that constant is for the dogfood install only; the test site needs the License page visible).
...
ssh witt-scafidi@100.102.251.125 'sudo tar -czf /tmp/pipe-pay-backup-$(date +%Y%m%d-%H%M%S).tar.gz -C /var/www/pipepay/wp-content/plugins pipe-pay && sudo -u www-data wp --path=/var/www/pipepay plugin deactivate pipe-pay && sudo rm -rf /var/www/pipepay/wp-content/plugins/pipe-pay && sudo unzip -q /var/www/pipepay/wp-content/uploads/woocommerce_uploads/pipe-pay-v1.6.4.zip -d /var/www/pipepay/wp-content/plugins/ && sudo chown -R www-data:www-data /var/www/pipepay/wp-content/plugins/pipe-pay && sudo find /var/www/pipepay/wp-content/plugins/pipe-pay -type f -exec chmod 644 {} \; && sudo find /var/www/pipepay/wp-content/plugins/pipe-pay -type d -exec chmod 755 {} \; && sudo -u www-data wp --path=/var/www/pipepay plugin activate pipe-pay && sudo systemctl reload php8.3-fpm'
...
## Local zips
- `/Users/wittscafidi/Desktop/Pipe Pay/pipe-pay-v1.6.4.zip` — **current customer-facing release AND what's installed on the dogfood gateway.** Security hardening release: Cloudflare-only IP detection, server-side amount/recipient cross-checks, forced TLS verification on license calls, per-billing-email upload cap, per-AI-provider key storage, PHP 8.0 minimum. Wired into all 4 WC products. Also staged on the server at `/var/www/pipepay/wp-content/uploads/woocommerce_uploads/`.
- `/Users/wittscafidi/Desktop/Pipe Pay/pipe-pay-v1.6.3.zip` — predecessor of 1.6.4. Superseded but kept as rollback option. Last build at PHP 7.4 minimum.

...
- WooCommerce: 10.7.0
- Pipe Pay plugin: **1.8.3** on the dogfood gateway and wired into all four WC products (`_product_version=1.8.3`, `_downloadable_files` → `pipe-pay-v1.8.3.zip`). v1.8.3 closes Phase 5: removes the `PIPEPAY_DISABLE_LICENSING` escape-hatch constant from the source code AND from `/var/www/pipepay/wp-config.php`. The dogfood now activates against itself with a real complimentary license (Unlimited tier, order #220, owned by pipepayadmin). With this ship, **all five phases of the anti-piracy plan (A/B/C/D + 5) are live and there is no longer any wp-config backdoor an attacker could read in the source and put in their own wp-config to bypass activation**. v1.8.2 added Phase B (per-customer download watermarking, forensic decoder admin page). v1.8.1 added Phase C (daily revalidation, three-state license model, drain semantics on revoke). v1.8.0 added Phases A + D (first-install gate, deterministic site fingerprinting). Public-facing site (release bar, changelog, footer ledger) still shows **v1.7.4** by design - `PIPEPAY_SITE_VERSION` reflects the most recent release with a published changelog narrative, not the actual dogfood version (per the docblock comment in `pipepay-child/functions.php`). The gap between public and dogfood is intentional: bug-fix and security patches between numbered "narrative" releases roll out to customers via auto-update without their own changelog entry. Plugin tags pushed up through `v1.8.3`. **Requires PHP 8.0** (was 7.4 pre-1.6.4). Source zips for every release live at `~/Desktop/Pipe Pay/pipe-pay-vX.Y.Z.zip`; v1.5.1 → v1.8.3 are also staged on the server at `/var/www/pipepay/wp-content/uploads/woocommerce_uploads/` for rollback. License-server URL is hardcoded to `https://pipepay.app/` in every build.

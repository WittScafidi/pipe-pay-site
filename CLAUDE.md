...
- WooCommerce: 10.7.0
- Pipe Pay plugin: **1.8.0** on the dogfood gateway and wired into all four WC products (`_product_version=1.8.0`, `_downloadable_files` → `pipe-pay-v1.8.0.zip`). v1.8.0 adds Phase A + D anti-piracy (first-install gate per-install token bound to `(api_key, instance, site_url)` re-verified on activate; tighter site fingerprinting; rejects replays from cloned WP installs). Public-facing site (release bar, changelog, footer ledger) still shows **v1.7.4** by design - `PIPEPAY_SITE_VERSION` reflects the most recent release with a published changelog narrative, not the actual dogfood version (per the docblock comment in `pipepay-child/functions.php`). The gap between public and dogfood is intentional: bug-fix and security patches between numbered "narrative" releases roll out to customers via auto-update without their own changelog entry. **In flight (uncommitted in plugin source):** v1.8.1 - anti-piracy Phase C (daily revalidation cron + revoke kill switch with drain semantics) and the em-dash sweep on top of v1.8.0; will land in the next tagged release. Plugin tags pushed up through `v1.7.9`; v1.8.0 and v1.8.1 are not yet tagged. **Requires PHP 8.0** (was 7.4 pre-1.6.4). The dogfood install has `PIPEPAY_DISABLE_LICENSING` set in `wp-config.php` (it hosts the license server itself; can't license against itself). Source zips for every release live at `~/Desktop/Pipe Pay/pipe-pay-vX.Y.Z.zip`; v1.5.1 → v1.8.0 are also staged on the server at `/var/www/pipepay/wp-content/uploads/woocommerce_uploads/` for rollback. License-server URL is hardcoded to `https://pipepay.app/` in every build.
